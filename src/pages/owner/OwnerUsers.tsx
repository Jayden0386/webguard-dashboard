...
import { Shield, Users, Ban, AlertTriangle, Activity, LogOut, Search, UserX, UserCheck } from "lucide-react";
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="h-9 bg-card border border-border rounded-lg pl-9 pr-4 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left p-4 text-xs font-medium text-muted-foreground uppercase">Username</th>
              ...
              <tbody>
                {filtered.map((u) => (
                  <tr key={u.id} className="border-b border-border last:border-0 hover:bg-accent/20 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${u.status === "active" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
                    ...
                    <td className="p-4 text-muted-foreground font-mono text-xs">{u.joined}</td>
                  </tr>
                ))}
              </tbody>
            </table>
